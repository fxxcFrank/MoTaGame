...
const createNewStory = (props) => {
    ...
    const clickCancel = () => {
        // setShowFlag(false);
        props.setCreateStoryModelFlag(false);
    }
    const onSubmit = (form) => {
        props.createNewStory(form);
        props.setCreateStoryModelFlag(false);
    }
    return (
        <Modal visible={props.createStoryModelFlag} title="新增故事" onOk={clickOk} onCancel={clickCancel} okText="创建" cancelText="取消" closable={false}>
            <Form ref={(val) => { setRefForm(val) }} onFinish={(form) => onSubmit(form)}>
                {/* <Item name="storyId" label="故事文件名" rules={[{required: true,message:"请输入故事文件名！"}]}>
                    <Input />
                </Item>
                        ...
                        {props.baseMapList.map((data,index) => {
                            return (
                                <Option className="CreateStory_LeftMenu_Option" value={data.lx}>
                                    ...
                                    position: "absolute", zIndex: index }} lx={data.lx} title={data.name} />
                                </Option>
                            )
                        })}
                    </Select>
